...
| 11.1 | **Code view — Phase 1: the anchored reader** | The #1 reported Code-view fix: clicking a code conversation used to yank the operator into Chat, abandoning the code. Killed that. `ReverseIndexPanel.svelte` evolved (not replaced — "reuse the existing composable pieces") with a new `anchored` mode, on ONLY while actually in Code view: one conversation is always focused and shown as a full card (real avatar/color via a new `agents` prop, kind badge colored by the SAME `EVENT_COLOR_VAR` palette piece 9 established), the rest collapse into a "‹ N more" strip of clickable pills, `j`/`k` steps focus between them. A locked scope header (`▤ whole file` / `▐ L44–49`) uses the existing `backToWholeFile` handler, unchanged. "open full thread ›" — the expanded card's own explicit link, wired to the SAME `openHitInChat` Chat's own ref-chip flow already uses — is the ONLY thing that still navigates to Chat; every other click (a row, a "more" pill) only ever changes FOCUS. Chat's own inline-ref-chip lookup is completely unaffected (`anchored` defaults `false`, keeping its exact pre-existing click-to-jump behavior). Caught and fixed a real focus-loss bug along the way: clicking a "more" pill unmounts it (it becomes the expanded card), silently dropping keyboard focus to `<body>` — a subsequent `j`/`k` press would never bubble back into the panel's own handler without explicitly re-focusing the panel root after every pill click. Ref: `redesign-mockups/11-code-view-BUILD-BRIEF.md`, `11-code-anchored-conversations.html`. Phases 2 (powered gutter) through 5 (sidebar timeline) are queued, one phase at a time, each screenshot-verified before the next. | ✅ done |
